...
@Component({
  selector: 'page-datafeednew',
  templateUrl: 'datafeednew.html'
})
export class DatafeednewPage {
  public selectedPair;
  public pairs = [];
  ...
  public connectSegment = 'predict';
  public account = 'ac1';
  BuyMarketSegment = 'market';
  apiStep = 'step1';
  ...
  ionViewWillEnter() {
    this.logger.log('TAB:', this.showlook);
  ...
  private processMarketTick(data) {
    ...
    this.ownCoinUSD =
      this.ownCoins * parseFloat(this.priceTick.lastTradePriceFormatted);
  }
...
*/
  ...
  changeBuyProc(proc) {
    this.buyMoney = (this.maximumUSD / 100) * proc;
    this.coinPrice =
      this.buyMoney / parseFloat(this.priceTick.lastTradePriceFormatted);
  }
  changeRange() {
    this.coinPrice =
      this.buyMoney / parseFloat(this.priceTick.lastTradePriceFormatted);
  ...
  changeBuyProcLim(proc) {
    this.buyMoneyLim = (this.maximumUSD / 100) * proc;
    this.coinPriceLim =
      this.buyMoneyLim / parseFloat(this.priceTick.lastTradePriceFormatted);
  }
  changeRangeLim() {
    this.coinPriceLim =
      this.buyMoneyLim / parseFloat(this.priceTick.lastTradePriceFormatted);
  ...
  enterMoney(ev) {
    ...
    if (money > this.maximumUSD) {
      this.coinPrice =
        this.maximumUSD / parseFloat(this.priceTick.lastTradePriceFormatted);
      this.buyMoney = this.maximumUSD;
    } else {
      this.buyMoney = money;
      this.coinPrice =
        this.buyMoney / parseFloat(this.priceTick.lastTradePriceFormatted);
  ...
  enterCoin(ev) {
    let money =
      ev.target.value * parseFloat(this.priceTick.lastTradePriceFormatted);
    if (money > this.maximumUSD) {
      this.coinPrice =
        this.maximumUSD / parseFloat(this.priceTick.lastTradePriceFormatted);
      this.buyMoney = this.maximumUSD;
    } else {
      this.coinPrice = ev.target.value;
      this.buyMoney = money;
    }
  }
  ...
  enterMoneyLim(ev) {
    ...
    if (money > this.maximumUSD) {
      this.coinPriceLim =
        this.maximumUSD / parseFloat(this.priceTick.lastTradePriceFormatted);
      this.buyMoneyLim = this.maximumUSD;
    } else {
      this.buyMoneyLim = money;
      this.coinPriceLim =
        this.buyMoneyLim / parseFloat(this.priceTick.lastTradePriceFormatted);
    }
  }
  ...
  enterCoinLim(ev) {
    ...
    if (money > this.maximumUSD) {
      ...
      this.coinPriceLim = ev.target.value;
      this.buyMoneyLim = money;
    }
  }

  apiNextStep(go: string) {
    this.apiStep = go == 'next' ? 'step2' : 'step1';
  }
}
